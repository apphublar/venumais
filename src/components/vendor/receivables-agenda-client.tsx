...
export function VendorReceivablesAgendaClient({
  filter,
  installments,
  store
}: {
    ...
    <>
          ...
          return (
            <section className="vendor-agenda-group" key={group.key}>
              ...
              {items.map((installment) => {
                ...
                return (
                  <VendorCard
                    className="vendor-agenda-row vendor-agenda-row-button"
                    key={installment.id}
      ...
      {active ? (
        <CobrancaSheet
          ...
          installmentStatus={receivableStatus(active.due_date) === "overdue" ? "overdue" : "open"}
